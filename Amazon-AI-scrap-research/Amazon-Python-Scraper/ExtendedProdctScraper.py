...
# CSV output file
output_file = 'amazonExtendedExtracted.csv'
...
headers = ['File Name', 'Product Title', 'Description', 'Product Insights', 'Pricing', 'Table Data', 'Feature Bullets', 
           'Product Overview', 'Brand Description', 'Product Details', 'Aplus Feature']
...
def extract_product_info(file_path):
    ...
    product_overview_tag = soup.find('div', id='productOverview_feature_div')
    product_overview = product_overview_tag.text.strip() if product_overview_tag else 'N/A'

    # Extract brand description (using class 'celwidget aplus-module launchpad-brand-description-left aplus-standard')
    brand_description_tag = soup.find('div', class_='celwidget aplus-module launchpad-brand-description-left aplus-standard')
    brand_description = brand_description_tag.text.strip() if brand_description_tag else 'N/A'

    # Extract product details
    product_details_tag = soup.find('div', id='productDetails_feature_div')
    product_details = product_details_tag.text.strip() if product_details_tag else 'N/A'

    # Extract aplus feature
    aplus_feature_tag = soup.find('div', id='aplus_feature_div')
    aplus_feature = aplus_feature_tag.text.strip() if aplus_feature_tag else 'N/A'

    return [file_path, title, description, insights, price, table_data, feature_bullets, 
            product_overview, brand_description, product_details, aplus_feature]
    ...
    if os.path.exists(csv_file):
        ...
        with open(csv_file, 'r', encoding='utf-8') as csvfile:
            ...
            try:
                ...
                # The file is empty, so there's no header to skip
                return processed_files

            # Add all previously processed file names to the set
            for row in reader:
