...
import { InventoryAvailabilityChip } from './AvailabilityChips';
import useScheduleStore from '../../../store/useScheduleStore';

const Inventory = ({ data, loading, error, onLocationChange }) => {
    ...
    const parseAvailability = (availability, quantityNeeded) => {
        return (
            <div className="flex flex-row justify-center gap-0.5">
                {formattedDateRange.map((date, index) => {
                    const quantity = getAvailabilityForDate(availability, date);
                    return <InventoryAvailabilityChip key={index} quantity={quantity} quantityNeeded={quantityNeeded} />;
                })}
            </div>
        );
    };
    ...
    const filteredData = data.filter(item => 
        (selectedInventoryId ? item.inventory_id === selectedInventoryId : true) && 
    ...
    const renderTable = (data) => (
        <table className="border-separate border-spacing-y-2 w-full">
            <thead>
                <tr>
                    <th className="border border-[#679436] p-2 text-center bg-gray-300 text-black font-normal h-20">Inventory ID</th>
                    <th className="border border-[#679436] p-2 text-center bg-gray-300 text-black font-normal h-20">Part Name</th>
                    ...
                    <th className="border border-[#679436] p-2 text-center bg-gray-300 text-black font-normal h-20">
                        ...
                        <div className="flex flex-row justify-center mt-2 gap-1.5">
                            {formattedDateRange.map((date, index) => (
                                <div key={index} className="flex flex-col bg-white rounded-lg items-center">
            ...
            <tbody>
                {data.map((row, rowIndex) => {
                    const mappedRow = {
                        ...
                        partName: row.part_required,
                        quantityNeeded: row.quantity_required,
                        availability: row.availability
                    };
                    return (
                        <tr key={rowIndex}>
                            <td className="border border-[#679436] p-2 text-center text-base h-16">{mappedRow.inventoryId}</td>
                            <td className="border border-[#679436] p-2 text-center text-base h-16">{mappedRow.partName}</td>
                            <td className="border border-[#679436] p-2 text-center text-base h-16">{mappedRow.location}</td>
                            <td className="border border-[#679436] p-2 text-center text-xs h-16">{parseAvailability(mappedRow.availability, mappedRow.quantityNeeded)}</td>
                        </tr>
                    );
                })}
            </tbody>
            ...
            <div className="flex gap-2 justify-end mb-4">
                <div className="relative h-6 w-36 flex items-center cursor-pointer border border-black">
                    ...
                    >
                        ...
                        {data.map(item => (
                            <option key={item.inventory_id} value={item.inventory_id}>{item.inventory_id}</option>
                        ))}
                    </select>
